...
for(var i=0; i< arr.length; i++){
  villages.push(JSON.parse(arr[i].innerHTML));
}

var ids = new Array();
var id_arr = document.getElementsByClassName('id-data');
for(var i=0; i< id_arr.length; i++){
  ids.push((JSON.parse(id_arr[i].innerHTML)));
}
...
//    center: map_center,
  ...
  //TODO: Use IDS to make asynchronous calls, because villages data is not passed through the HTML
  //TODO: Note: this loop may be completely useless; check whther it is actually used and remove if necessary
    var index = 0
  ...
  for (var v in villages) {
    ...
    var villageInfo = new google.maps.InfoWindow({
      ...
      +'<h3>' + village.name + '&nbsp<a href="ml-ws/'+village.id+'">' 
      + '<img width="15px" padding-left="30px" src="../static/img/download.svg"></a>&nbsp<a href="ml-ws/analytics/'+village.id+'">' 
      + '<img width="15px" padding-left="60px" src="../static/img/chart.jpg"></a></h3>'
      + '<i>Updated: '+village.ts+'</i>'
      // + '<a href="#">Download data</a>'
      + '<br /><br />'
        + '<table>'
          +'<tr><td>Air temperature&nbsp&nbsp&nbsp&nbsp&nbsp</td><td>'+(parseFloat(village.t1)).toFixed(1)+'&deg;C</td></tr>'
    ...
    //TODO: for every circle, make an asynchronous request and add the data
...
try {
              console.log("GET_WEATHER_DATA:" + i)

     var xhttp = new XMLHttpRequest();
     
              var target_url = "http://127.0.0.1:5000/dashboard/get-data/" + i.toString()
              console.log("TARGET_URL:" + target_url)

              xhttp.open("GET", target_url, false);
              xhttp.send();
//              console.log(xhttp.responseText);
              json_id = xhttp.responseText
              json_id = JSON.parse(json_id)
//              console.log("JSON")
//              console.log(json_id)
              ...
              all_infos.push(villageInfo);
              console.log("all_infos.length:"+all_infos.length);
            ...
            all_circles.push(villageCircle);
              console.log("all_circles.length:"+all_circles.length);

            // add listener to last circle
            index = all_circles.length  - 1
            google.maps.event.addListener(all_circles[index], 'click', function(innerKey) {
              return function() {
                all_infos[innerKey].open(map, all_circles[innerKey]);
              }
...
//            index++;
            ...
            console.log("Could not get data for id " + i.toString() + ":" + err)
          }
          //TODO: Create a loading bar and figure out how to update it once data is retrieved
