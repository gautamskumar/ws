...
function initMap() {
  ...
  for (var v in villages) {
    ...
    var villageInfo = new google.maps.InfoWindow({
      ...
      + '<br /><br />'
        + '<table>'
          +'<tr><td>1-Day Forecast&nbsp&nbsp&nbsp&nbsp&nbsp</td><td>'+village.oneDayForecast+' kWh</td></tr>'
          +'<tr><td>25-Year Forecast&nbsp&nbsp&nbsp&nbsp&nbsp</td><td>'+village.tfiveYearForecast+' kWh</td></tr>'
        + '</table>'
        + '<table style="margin-top: 18px;">'
          +'<tr><td>Hist. Volatility&nbsp&nbsp&nbsp&nbsp&nbsp</td><td>'+village.histVolatility+'%</td></tr>'
        + '</table>'
      + '</div>'
    });
...
function circleColor(unit) {
  pct_dep = parseFloat(unit.data.pct_dep);
  tarrif  = parseFloat($('input[name="tarrif"]').val());
  rate    = parseFloat($('input[name="rate"]').val());
  months  = parseFloat($("#months option:selected").val());
  orange  = parseFloat($('input[name="orange"]').val())/100.0;
  red     = parseFloat($('input[name="red"]').val())/100.0;
  proj    = parseFloat(unit.data.projOutput.replace(/,/g , ''));
  cost    = parseFloat(unit.data.cost);
  size    = parseFloat(unit.data.size);
  predReturns = tarrif*((1+rate)**(months/12))*parseFloat(unit.data.tfiveYearForecast.replace(/,/g , ''));
  // if ($('#parameter').val() == "oneDayForecast") {
  //   if (parseFloat(unit.data.oneDayForecast) <= (red*proj/365*size)) {
  //     return "red";
  //   }
  //   else if (parseFloat(unit.data.oneDayForecast) <= (orange*proj/365*size)) {
  //     return "orange";
  //   }
  //   else {
  //     return "green";
  //   }
  // }
  // if ($('#parameter').val() == "oneYearForecast") {
  //   if (parseFloat(unit.data.oneYearForecast.replace(/,/g , '')) <= parseFloat(red*proj*size)) {
  //     return "red";
  //   }
  //   else if (parseFloat(unit.data.oneYearForecast.replace(/,/g , '')) <= parseFloat(orange*proj*size)) {
  //     return "orange";
  //   }
  //   else {
  //     return "green";
  //   }
  // }
  if ($('#parameter').val() == "tfiveYearForecast") {
    // Our forecast
    console.log(unit.data.name);  
    console.log(predReturns);
    console.log(cost);
    // Their forecast
    
    var projReturns = tarrif*((1+rate/months)**(months/12))*parseFloat(proj*((1-Math.pow((1-pct_dep),25))/(1-(1-pct_dep)))*size);
    console.log(projReturns);

    if (cost >= orange*predReturns) {
      return "orange";
    }
    else if (cost >= red*predReturns) {
      return "red";
    }
    else {
      return "green";
  ...
  // if ($('#parameter').val() == "weekActuals") {
  //   if (parseFloat(unit.data.weekActuals) <= parseFloat(red*proj/3.65/24)) {
  //     return "red";
  //   }
  //   else if (parseFloat(unit.data.weekActuals) <= parseFloat(orange*proj/3.65/24)) {
  //     return "orange";
  //   }
  //   else {
  //     return "green";
  //   }
  // }
  // if ($('#parameter').val() == "totalActuals") {
  //   if (parseFloat(unit.data.totalActuals) <= parseFloat(red*proj/3.65/24)) {
  //     console.log(parseFloat(unit.data.totalActuals));
  //     return "red";
  //   }
  //   else if (parseFloat(unit.data.totalActuals) <= parseFloat(orange*proj/3.65/24)) {
  //     console.log(parseFloat(unit.data.totalActuals));
  //     return "orange";
  //   }
  //   else {
  ...
  //   }
  // }
  if ($('#parameter').val() == "histVolatility") {
    if (parseFloat(unit.data.histVolatility)/100.0 >= parseFloat(red)) {
      return "red";
    }
    else if (parseFloat(unit.data.histVolatility)/100.0 >= parseFloat(orange)) {
      console.log(parseFloat(unit.data.histVolatility)/100.0);
      return "orange";
    }
    else {
      return "green";
